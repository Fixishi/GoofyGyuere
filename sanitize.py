...
def _SanitizeTag(t):
  ...
  disallowed_attributes = [
      ...
      'onselect', 'onsubmit', 'onunload', 'onmouseover', 'onmouseenter', 'href', 'src', 'style'
  ]
...
def _GenerateXsrfToken(self, cookie, action):
        """Generates an XSRF token tied to a specific action."""
        timestamp = str(time.time())
        token = hashlib.sha256((self.cookie_secret + cookie + action + timestamp).encode()).hexdigest()
        return timestamp + "|" + token

def _VerifyXsrfToken(self, cookie, action, action_token):
      """Verifies an XSRF token included in a request."""
      (action_time, action_hash) = action_token.split("|", 1)

      now = time.time()
      if now - 3600 > float(action_time): 
        return False

      expected_token = hashlib.sha256((self.cookie_secret + cookie + action + action_time).encode()).hexdigest()
      return action_hash == expected_token
